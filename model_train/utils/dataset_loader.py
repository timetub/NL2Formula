...
from .nl2formula import nl2formula_add_serialized_schema,nl2formula_pre_process_function
...
def load_dataset(
    ...
    _nl2formula_dataset_dict: Callable[[], DatasetDict]=lambda :datasets.load.load_dataset(
        path=data_args.dataset_paths['nl2vis'],cache_dir=model_args.cache_dir
    )
    _nl2formula_metric: Callable[[],Metric]=lambda : datasets.load.load_metric(
        path=data_args.metric_paths["nl2vis"], config_name=data_args.metric_config,
        test_suite_db_dir=data_args.test_suite_db_dir
    )
    _nl2formula_add_serialized_schema=lambda ex: nl2formula_add_serialized_schema(
        ex=ex,
        data_training_args=data_training_args,
    )

    _nl2formula_pre_process_function = lambda batch, max_source_length, max_target_length: nl2formula_pre_process_function(
        batch=batch,
        max_source_length=max_source_length,
        max_target_length=max_target_length,
        data_training_args=data_training_args,
    ...
    elif data_args.dataset =="nl2vis":
        metric=_nl2formula_metric()
        dataset_splits=prepare_splits(
            dataset_dict=_nl2formula_dataset_dict(),
            add_serialized_schema=_nl2formula_add_serialized_schema,
            pre_process_function=_nl2formula_pre_process_function,
            **_prepare_splits_kwargs,
        )
    else:
